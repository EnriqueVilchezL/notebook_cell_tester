...
- `ColabTestFramework` — orchestrates the full workflow:
  ...
  2. `run_tests(tests)` dispatches each `TestCase` to the appropriate method based on `test_type`, then stamps `result.description = test.description` before appending.
  3. `display_results()` renders an HTML table via `IPython.display`.
...
**Namespace-only tests:** `variable` and `type_check`-on-variable tests only need the IPython namespace. `run_tests` runs these even when `load_last_cell()` returns an empty string (e.g., when the student hasn't run their cell yet).
